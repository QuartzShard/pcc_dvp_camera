...
pub const fn init_regs(
    ...
    let resolution = resolution_regs(h_res, v_res, image_scale, 0);
...
/// Generates timing control registers for a centered sensor window at the given
/// output resolution and scale factor.
///
/// Configures the sensor readout window, ISP output size, ISP offsets, and
/// frame timing (HTS/VTS). The readout window is centered on the sensor array.
///
/// HTS (total horizontal size) is set to the readout window width plus [`H_BLANKING`].
/// VTS (total vertical size) is set to the readout window height plus [`V_BLANKING`],
/// plus `extra_vblank` additional lines.
///
/// `extra_vblank` adds extra vertical blanking lines to the frame, increasing the
/// time between frames. Each extra line adds HTS PCLK cycles to the total frame
/// time, so the additional delay per frame is `extra_vblank * HTS / PCLK` seconds.
/// For example, with HTS=1564 and PCLK=3MHz, each extra line adds ~0.52ms.
pub const fn resolution_regs(
    h_res: usize,
    v_res: usize,
    image_scale: usize,
    extra_vblank: usize,
    ...
    let vts: u16 = y_end - y_start + V_BLANKING as u16 + extra_vblank as u16;
